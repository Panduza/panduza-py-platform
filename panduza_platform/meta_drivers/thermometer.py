...
class MetaDriverThermometer(PlatformDriver):
    """Abstract Driver with helper class to manage ThermoMeter interface
    """
    ...
    async def _PZA_DRV_THERMOMETER_read_measure_value(self):
        """
        """
    ...
    async def __polling_task(self):
        ...
        while self.alive:
            ...
            await self._update_attributes_from_dict({
                "measure": {
                    "value": await self._PZA_DRV_THERMOMETER_read_measure_value()
                }
            })
